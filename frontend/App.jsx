    ...
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: 'linear-gradient(145deg, #f7efe4, #e8d5bd)', padding: 20 }}>
      <form onSubmit={submit} style={{ ...cardStyle, width: '100%', maxWidth: 520, padding: 28 }}>
        <div style={{ textAlign: 'center', marginBottom: 22 }}>
          ...
          <div style={{ color: palette.muted, marginTop: 6 }}>Menu và kho hàng tách riêng hoàn toàn, cùng đồng bộ MongoDB</div>
        </div>
      ...
      <div style={{ display: 'grid', gap: 16, gridTemplateColumns: '1.2fr 1fr' }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            ...
            <div style={{ border: `1px solid ${palette.line}`, borderRadius: 14, padding: 12 }}>
              <div style={{ color: palette.muted }}>Kho hàng riêng</div>
...
function WarehouseAdmin({ warehouseItems, refreshWarehouse, isAdmin }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 16, gridTemplateColumns: '1fr 1.55fr' }}>
      <div style={cardStyle}>
        <SectionTitle title={editing ? 'Sửa hàng kho' : 'Nhập hàng kho thủ công'} subtitle="Kho hàng là khu riêng, đồng bộ MongoDB và không liên quan menu" />
        {!isAdmin ? (
          ...
          <form onSubmit={submit} style={{ display: 'grid', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              {editing ? <button type="button" style={mutedButton} onClick={() => { setEditing(null); setForm(emptyForm); }}>Hủy</button> : null}
            </div>
          </form>
        )}
      </div>
      ...
      <div style={cardStyle}>
        <SectionTitle title="Danh sách kho hàng riêng" subtitle="Dữ liệu kho đã đồng bộ giữa nhiều thiết bị" />
        <div style={{ marginBottom: 12 }}>
...
export default function App() {
  ...
  async function refreshWarehouse() {
    await Promise.all([loadWarehouse(), loadWarehouseLogs()]);
  }
